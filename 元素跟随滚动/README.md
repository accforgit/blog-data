...
如果你想让右侧滚动框也平滑滚动，也是可以做到的，当每次监听到 `wheel`事件的时候，也别管它相比于上次是差了`100px`还是 `50px`的，始终都让右侧的跟随滚动框按照 `10px`（或者再稍大点或者少小点的跨度，只要给人视觉上的感受是平滑滚动并且延迟不是太大就行了）来滚动，连续滚动 `10`次，那就是`100px`了，同样能到达准确的位置，例如如下代码：
```
